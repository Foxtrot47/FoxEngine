...
Mesh::Mesh(Graphics& gfx, std::vector<Vertex> vertices, std::vector<unsigned short> indices, std::wstring texturePath)
{
	...
	AddBind(std::make_unique<Texture>(gfx, texturePath));
	...
	AddBind(std::make_unique<PixelShader>(gfx, GetExecutableDirectory() + L"\\PhongPS.cso"));
